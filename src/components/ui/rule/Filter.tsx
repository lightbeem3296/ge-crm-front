...
import { ComparableFilterCondition, comparableFilterConditionCodes, comparableFilterConditionMappings, FilterType, objectFilterConditionCodes, objectFilterConditionMappings, stringFilterConditionCodes, stringFilterConditionMappings, StringFilterField } from "@/types/filter";
...
export default function RuleFilterComponent({ field, label, type, ruleIndex, conditionIndex, rule, setRule }: FilterComponentProps) {
  ...
  useEffect(() => {
    ...
    if (atomCondition.filter) {
      const filterField2 = atomCondition.filter[field];
      console.log("loading", field, filterField2);
      if (filterField2) {
        if (filterField2.value instanceof Array) {
          setFilterValues(filterField2.value);
        } else {
          setFilterValue(filterField2.value);
        }
        setFilterCondition(filterField2.condition);
        if (filterField2 instanceof StringFilterField) {
          setFilterCaseSensitive(filterField2.case_sensitive);
        }
      }
    }
          ...
          ? {
            ...
            condition: {
              ...
              conditions: atom_rule.condition.conditions.map((atom_condition, c_index) =>
                c_index === conditionIndex
                  ? {
                    ...atom_condition,
                    filter: {
                      ...atom_condition.filter,
                      [field]: filterField,
                    },
                  }
                  : atom_condition
